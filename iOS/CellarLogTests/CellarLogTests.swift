import XCTest
@testable import CellarLog

@MainActor
final class CellarLogTests: XCTestCase {
    var store: Store!
    ...
    func testAddEntryIncreasesCount() {
        ...
        store.add(BottleEntry(title: "Test", detail1: "A", detail2: "B", note: "", date: Date()))
        XCTAssertEqual(store.entries.count, before + 1)
    }

    func testDeleteEntryDecreasesCount() {
        store.add(BottleEntry(title: "ToDelete", detail1: "A", detail2: "B", note: "", date: Date()))
        let before = store.entries.count
    ...
    func testCannotAddMoreWhenAtLimitAndNotPro() {
        while store.entries.count < Store.freeLimit {
            store.add(BottleEntry(title: "Filler", detail1: "A", detail2: "B", note: "", date: Date()))
        }
    ...
    func testUpdateEntryChangesTitle() {
        store.add(BottleEntry(title: "Original", detail1: "A", detail2: "B", note: "", date: Date()))
